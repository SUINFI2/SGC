...
class Venta extends Model {
  ...
  async totalCobrado() {
    ...
    if (this.cobros.length > 0 && !this.confirmCobro) {
      var sum =0;
      for(let i =1; i<this.cobros.length; i++){
        sum += this.cobros[i].monto;
      }
      return sum;
    }
    if(this.cobros.length===0){return 0;}
    return total;
  }
  async cntCobros() {
    if (this.cobros.length > 0) {
      return this.cobros.reduce((total, item) => {
        return total + 1;
      }, 0);
    }
    return 0;
